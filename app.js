...
async function loadMoties() {
  try {
    ...
    if (!r.ok) throw new Error(r.status);
    moties = await r.json();
    renderMoties(); renderMotiesVisuals(); populatePartijFilter(); renderOvMot();
    // NIEUW #12
...
async function loadCamerasEnSluitingen() {
  ...
  renderOverlapLijst();
  renderCamPerWeekChart();
  renderWoningNuGeslotenLijst();
}
...
// NIEUW: geeft aan of "vandaag" (nog niet gearchiveerd in percentageHistorie)
// mag meetellen bij de huidige weergave. Zonder maandfilter altijd ja; met
// een maandfilter alleen als de gekozen maand ook de huidige maand is —
// anders zou een oude maand ten onrechte vandaag se cijfers erbij krijgen.
function vandaagTeltMeeInMaandFilter() {
  if (!_uvMaandFilter) return true;
  return _uvMaandFilter === new Date().toISOString().slice(0, 7);
}
...
function renderUvDagChart() {
  ...
  if (vandaagKey && vandaagTeltMeeInMaandFilter()) {
    dagMap[vandaagKey] = {
  ...
  const maxC = Math.max(...dagLijst.map(([,d]) => d.cancelled), 1);
  ...
  dagEl.innerHTML = `
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;padding:4px 0 12px;font-size:10px;color:var(--muted);">
      ...
      <span>${labelSindsDatum(eersteDatumMetVeld('totaal'))}</span>
    </div>`;
}

// NIEUW: uitvalpercentage per dag — zelfde databron en periodefilter als
// renderUvDagChart() hierboven, maar dan als percentage (cancelled/totaal)
// i.p.v. absoluut aantal. Het totaal aantal ritten per dag zit al in
// percentageHistorie, dus dit vereist geen nieuwe berekening in de scraper.
function renderUvDagPctChart() {
  const pctEl = document.getElementById('uvDagPctChart');
  if (!pctEl) return;
  ...
  if (dagLijst.length < 1) {
    pctEl.innerHTML = '<div class="viz-empty">Onvoldoende data voor deze periode</div>';
    return;
  }
  ...
  const PAD = { t: 10, r: 16, b: 36, l: 36 };
  const pW = W - PAD.l - PAD.r, pH = H - PAD.t - PAD.b;
  const pcts = dagLijst.map(([, d]) => (d.cancelled / d.totaal) * 100);
  ...
  const bars = dagLijst.map(([datum, d], i) => {
    const pct  = (d.cancelled / d.totaal) * 100;
    const x    = PAD.l + i * slot + (slot - barW) / 2;
    const yB   = PAD.t + pH;
    const hPct = Math.round((pct / maxP) * pH);
    const dd   = datum.slice(5);
    const label = (i % labelStap === 0)
      ? `<text x="${x+barW/2}" y="${H-4}" text-anchor="middle" font-size="9" fill="var(--muted)">${dd}</text>`
      : '';
    return `
      <rect x="${x}" y="${yB - hPct}" width="${barW}" height="${hPct}" fill="var(--teal)" opacity="0.82" rx="1">
        <title>${datum}: ${pct.toFixed(1)}% (${d.cancelled} van ${d.totaal})</title></rect>
      ${label}`;
  }).join('');

  const yTicks = [0, maxP/2, maxP].map(v => {
    const y = PAD.t + pH - (v/maxP)*pH;
    return `<line x1="${PAD.l}" y1="${y}" x2="${PAD.l+pW}" y2="${y}" stroke="var(--rule)" stroke-width="0.5"/>
            <text x="${PAD.l-4}" y="${y+3}" text-anchor="end" font-size="9" fill="var(--muted)">${v.toFixed(0)}%</text>`;
  }).join('');
  ...
  pctEl.innerHTML = `
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;padding:4px 0 12px;font-size:10px;color:var(--muted);">
      <span style="display:flex;align-items:center;gap:4px;"><span style="width:8px;height:8px;background:var(--teal);opacity:.82;display:inline-block;"></span>Uitvalpercentage</span>
      <span>${labelSindsDatum(eersteDatumMetVeld('totaal'))}</span>
    </div>`;
}
...
function bouwUitvalPerMaand() {
  ...
  // maar alleen als vandaag ook binnen het gekozen maandfilter valt.
  const vandaagKey = Object.keys(totaalTeller)[0];
  if (vandaagKey && vandaagTeltMeeInMaandFilter()) {
...
function renderUitvalPerMaandCharts() {
  ...
  const sindsLabel = labelSindsDatum(eersteDatumMetVeld('totaal'));

  if (aantalEl) {
    const maxV = Math.max(...maandLijst.map(([, d]) => d.cancelled), 1);
    ...
    }).join('') + `<div style="padding:8px 20px 0;font-size:10px;color:var(--muted);">${sindsLabel}</div>`;
  }

  if (pctEl) {
    const pctPerMaand = maandLijst.map(([maand, d]) => [maand, d.totaal ? d.cancelled / d.totaal * 100 : 0]);
...
function renderUitvalPctPerLijnChart() {
  ...
  if (vandaagKey && totaalTeller[vandaagKey]?.totaal_per_lijn) {
    ...
    uitval.filter(r => r.status === 'cancelled').forEach(r => {
  ...
  const sindsDatum = labelSindsDatum(eersteDatumMetVeld('totaal_per_lijn'));
  el.innerHTML = rijen.map(r => `<div class="viz-bar-row">
    ...
    + `<div style="padding:8px 20px 0;font-size:10px;color:var(--muted);">Gebaseerd op ${dagenMetData} dag(en) met totaal_per_lijn-data · lijnen met &lt;10 ritten weggelaten. ${sindsDatum}</div>`;
}
...
function renderUitvalVergelijking() {
  ...
  totEl.innerHTML = reeksen.map(r => `
    ...
    + `<div style="padding:6px 10px 0;font-size:10px;color:var(--muted);">${labelSindsDatum(eersteDatumMetVeld('per_lijn'))}</div>`;
...
function renderUitval() {
  ...
  // maar alleen als vandaag ook binnen het gekozen maandfilter valt. Anders
  // zou een oude, afgesloten maand ten onrechte de cijfers van vandaag erbij
  // krijgen.
  const vandaagKey = Object.keys(totaalTeller)[0];
  const vandaagTeltMee = vandaagTeltMeeInMaandFilter();
  ...
  const periodeEinde = vandaagTeltMee ? 'vandaag' : (alleDatums[alleDatums.length - 1] || '');
  document.getElementById('uvPeriode').textContent =
    alleDatums.length ? `${alleDatums[0]} t/m ${periodeEinde}` : (vandaagKey && vandaagTeltMee ? vandaagKey + ' t/m vandaag' : 'geen data');
...
function renderUitvalLijst() {
  ...
  let f = uitval.filter(r => r.status === 'cancelled');
...
// ══════════════════════════════════════════════════════════════════════════
// NIEUW — ACTUELE MELDINGEN (Uitval-tab)
// De storingsmeldingen die EBS bovenaan de departureboard toont (bv. "Door
// technische storingen kunnen ritten op lijn 67 uitvallen", geldig 4-22 aug)
// zijn tot nu toe nooit opgeslagen — de scraper sloeg alleen de losse
// vertrekregels op. Dit haalt data/ebs_meldingen.json op, geschreven door de
// nieuwe parse_meldingen()-functie in scrape_ebs.py.
// mogelijk_lijnen is best-effort tekstherkenning (regex op "lijn(en)"/"bus(sen)"
// gevolgd door cijfers) — geen gegarandeerd correcte lijst, dus expliciet zo
// gelabeld i.p.v. als harde data gepresenteerd.
// ══════════════════════════════════════════════════════════════════════════
async function loadEbsMeldingen() {
  try {
    const r = await fetch('./data/ebs_meldingen.json');
    if (!r.ok) throw new Error(r.status);
    ebsMeldingen = await r.json();
    renderEbsMeldingen();
  } catch (e) {
    ebsMeldingen = [];
    const el = document.getElementById('ebsMeldingenLijst');
    if (el) {
      el.innerHTML = e.message === '404'
        ? '<div class="empty">Nog geen meldingendata — draai eerst de bijgewerkte scrape_ebs.py.</div>'
        : `<div class="error-msg">Fout bij laden ebs_meldingen.json: ${e.message}</div>`;
    }
  }
}

function renderEbsMeldingen() {
  const el = document.getElementById('ebsMeldingenLijst');
  if (!el) return;
  if (!ebsMeldingen.length) { el.innerHTML = '<div class="empty">Geen actuele meldingen.</div>'; return; }

  const nu = new Date().toISOString();
  const gesorteerd = [...ebsMeldingen].sort((a, b) => (b.starttijd || '').localeCompare(a.starttijd || ''));

  el.innerHTML = gesorteerd.map(m => {
    const isActief = m.eindtijd && m.eindtijd >= nu;
    const periodeStr = [m.starttijd, m.eindtijd].filter(Boolean).map(t => t.replace('T', ' ').slice(0, 16)).join(' t/m ');
    const lijnenHtml = (m.mogelijk_lijnen || []).length
      ? `<div style="margin-top:4px;font-size:11px;color:var(--muted);">Mogelijk betrokken lijnen (automatisch herkend, controleer handmatig): ${
          m.mogelijk_lijnen.map(l => `<span class="badge badge-teal" style="font-size:10px;">${esc(l)}</span>`).join(' ')
        }</div>`
      : '';
    return `<div class="bk-item">
      <div class="bk-top"><div style="flex:1;">
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:4px;">
          <span class="badge ${isActief ? 'badge-hold' : 'badge-teal'}" style="font-size:10px;">${isActief ? 'Actief' : 'Verlopen'}</span>
          ${periodeStr ? `<span style="font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--muted);">${esc(periodeStr)}</span>` : ''}
        </div>
        <div style="font-size:13px;font-weight:600;">${esc(m.titel || m.bericht || '')}</div>
        ${m.bericht && m.bericht !== m.titel ? `<div class="bk-desc">${esc(m.bericht)}</div>` : ''}
        ${lijnenHtml}
        ${(m.haltes_gezien || []).length ? `<div style="margin-top:3px;font-size:10px;color:var(--muted);">Gezien op: ${m.haltes_gezien.map(esc).join(', ')}</div>` : ''}
      </div></div>
    </div>`;
  }).join('');
}
...
// ══════════════════════════════════════════════════════════════════════════
// Overzicht-tab: "laatste activiteiten"-kaarten per databron. Vervangt de
// oude onderwerpenranglijst (keyword-telling) door een concreet overzicht:
// wat is er recent gebeurd, en wat staat er nog aan te komen.
// ══════════════════════════════════════════════════════════════════════════
...
function renderOvVerg() {
  ...
  const gesorteerd = [...vergaderingen].sort((a, b) => (b.datum || '').localeCompare(a.datum || ''));
  const recent    = gesorteerd.filter(v => (v.datum || '') <= vandaag).slice(0, 4);
  const aankomend = gesorteerd.filter(v => (v.datum || '') > vandaag)
    .sort((a, b) => (a.datum || '').localeCompare(b.datum || '')).slice(0, 3);
  ...
  let html = '';
  if (aankomend.length) {
    html += `<div style="font-size:10px;color:var(--muted);font-weight:700;text-transform:uppercase;padding:2px 0 4px;">Aankomend</div>`
      + aankomend.map(item).join('')
      + `<div style="font-size:10px;color:var(--muted);font-weight:700;text-transform:uppercase;padding:10px 0 4px;">Recent</div>`;
  }
  html += recent.map(item).join('') || '<div class="empty">Geen data</div>';
  document.getElementById('ovVerg').innerHTML = html;
}
...
function renderBekendmakingenDashboard() {
  ...
  // Woningsluitingen per dag i.p.v. per maand — met ~1 maand aan data (vanaf
  // 1 juli) is maand-granulariteit te grof om iets af te lezen.
  renderWoningPerDagChart();
...
// ══════════════════════════════════════════════════════════════════════════
// NIEUW — WONINGSLUITINGEN PER DAG (i.p.v. per maand)
// Met ~1 maand aan data (bijgehouden vanaf 1 juli) is maand-granulariteit
// nauwelijks informatief — één blokje per maand. Per dag, als staafdiagram,
// is op deze schaal veel bruikbaarder. Zelfde SVG-stijl als de EBS-uitval-
// per-dag-chart.
// ══════════════════════════════════════════════════════════════════════════
function renderWoningPerDagChart() {
  const container = document.getElementById('bkWoningChart');
  if (!container) return;
  const perDag = {};
  woningsluitingen.forEach(w => { if (w.datum) perDag[w.datum] = (perDag[w.datum] || 0) + 1; });
  const dagLijst = Object.entries(perDag).sort((a, b) => a[0].localeCompare(b[0]));
  if (dagLijst.length < 1) { container.innerHTML = '<div class="viz-empty">Onvoldoende data</div>'; return; }

  const W = 500, H = 200, PAD = { t: 10, r: 16, b: 30, l: 36 };
  const pW = W - PAD.l - PAD.r, pH = H - PAD.t - PAD.b;
  const maxC = Math.max(...dagLijst.map(([, n]) => n), 1);
  const slot = pW / dagLijst.length;
  const barW = Math.max(4, Math.floor(slot * 0.6));
  const labelStap = Math.max(1, Math.ceil(dagLijst.length / 12));

  const bars = dagLijst.map(([datum, n], i) => {
    const x = PAD.l + i * slot + (slot - barW) / 2;
    const h = Math.round((n / maxC) * pH);
    const y = PAD.t + pH - h;
    const label = (i % labelStap === 0)
      ? `<text x="${x + barW / 2}" y="${H - 8}" text-anchor="middle" font-size="9" fill="#6B7280">${datum.slice(5)}</text>`
      : '';
    return `<rect x="${x}" y="${y}" width="${barW}" height="${h}" fill="#006B7B" opacity="0.82" rx="1"><title>${datum}: ${n}</title></rect>${label}`;
  }).join('');

  const yTicks = [0, Math.ceil(maxC / 2), maxC].map(v => {
    const y = PAD.t + pH - (v / maxC) * pH;
    return `<text x="${PAD.l - 6}" y="${y + 4}" text-anchor="end" font-size="10" fill="#6B7280">${v}</text>`;
  }).join('');

  container.innerHTML = `<svg viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%;">
    <line x1="${PAD.l}" y1="${PAD.t}" x2="${PAD.l}" y2="${PAD.t+pH}" stroke="#D8D5CE" stroke-width="1"/>
    <line x1="${PAD.l}" y1="${PAD.t+pH}" x2="${PAD.l+pW}" y2="${PAD.t+pH}" stroke="#D8D5CE" stroke-width="1"/>
    ${yTicks}${bars}
  </svg>`;
}

function renderOvBk() {
  const camActiefItems = camerasActief.map(c => ({
    datum: c.start, titel: `Cameratoezicht ${c.camera} — actief`, link: (c.periodes?.[0]?.link) || '#', type: 'Camera',
  }));
  // NIEUW: camerasGeschiedenis toont nu de eind-datum (wanneer de camera weer
  // is uitgeschakeld) i.p.v. de start-datum — dat is immers de "sluiting"
  // waar dit kaartje naar genoemd is.
  const camGeschiedenisItems = camerasGeschiedenis.map(c => ({
    datum: c.eind || c.start, titel: `Cameratoezicht ${c.camera} — beëindigd`, link: (c.periodes?.[0]?.link) || '#', type: 'Camerasluiting',
  }));
  const woningItems = woningsluitingen.map(w => ({
    datum: w.datum, titel: w.titel, link: w.link || '#', type: 'Woningsluiting',
  }));
  const combined = [...camActiefItems, ...camGeschiedenisItems, ...woningItems]
    .filter(x => x.datum)
...
// NIEUW: laatste stemmingen + hoe ze verliepen (Overzicht-tab)
function renderOvStem() {
  const el = document.getElementById('ovStem');
  if (!el) return;
  const gesorteerd = [...stemmingen].sort((a, b) => (b.datum || '').localeCompare(a.datum || ''));
  el.innerHTML = gesorteerd.slice(0, 5).map(s => {
    const uitslag = s.uitslag_tekst || s.uitslag || 'nog geen uitslag';
    const kleur = /aangenomen/i.test(uitslag) ? 'var(--go)' : /verworpen/i.test(uitslag) ? 'var(--stop)' : 'var(--muted)';
    return `
      <div class="mini-item">
        <div class="mini-date">${fmtDate(s.datum, 'short')}</div>
        <div>
          <div class="mini-title">${esc(s.titel)}</div>
          <div class="mini-type" style="color:${kleur};">${esc(uitslag)}</div>
        </div>
      </div>`;
  }).join('') || '<div class="empty">Geen data</div>';
}

// NIEUW: laatste moties (Overzicht-tab)
function renderOvMot() {
  const el = document.getElementById('ovMot');
  if (!el) return;
  ...
  el.innerHTML = gesorteerd.slice(0, 5).map(m => {
    ...
    return `
      <div class="mini-item">
        ...
        <div>
          <div class="mini-title">${esc(m.titel)}</div>
          <div class="mini-type"><span class="badge ${cls}">${label}</span> ${esc(m.partij || '')}</div>
        </div>
      </div>`;
  }).join('') || '<div class="empty">Geen data</div>';
}

// NIEUW: laatste EBS-cijfers (Overzicht-tab) — hergebruikt de cumulatieve
// stats die renderUitval() al berekent, om dubbele logica te voorkomen.
function renderOvEbs() {
  const el = document.getElementById('ovEbs');
  if (!el) return;
  const totaal       = document.getElementById('uvTotaal')?.textContent || '—';
  const totaalRitten = document.getElementById('uvTotaalRitten')?.textContent || '';
  const pct          = document.getElementById('uvPct')?.textContent || '—';
  const periode      = document.getElementById('uvPeriode')?.textContent || '';
  const topLijn      = document.getElementById('uvTopLijn')?.textContent || '—';
  const topLijnSub   = document.getElementById('uvTopLijnSub')?.textContent || '';
  el.innerHTML = `
    <div style="display:flex;flex-wrap:wrap;gap:20px;padding:4px 0 8px;">
      <div>
        <div style="font-size:11px;color:var(--muted);">Uitgevallen ritten</div>
        <div style="font-size:24px;font-weight:700;">${totaal}</div>
        <div style="font-size:11px;color:var(--muted);">${totaalRitten}</div>
      </div>
      <div>
        <div style="font-size:11px;color:var(--muted);">Uitvalpercentage</div>
        <div style="font-size:24px;font-weight:700;color:var(--stop);">${pct}</div>
      </div>
      <div>
        <div style="font-size:11px;color:var(--muted);">Meest getroffen lijn</div>
        <div style="font-size:24px;font-weight:700;">${topLijn}</div>
        <div style="font-size:11px;color:var(--muted);">${topLijnSub}</div>
      </div>
    </div>
    <div style="font-size:10px;color:var(--muted);">${periode}</div>`;
}
...
// ══════════════════════════════════════════════════════════════════════════
// NIEUW — AANTAL CAMERA'S PER WEEK
// Niet-cumulatief: hoeveel camera's waren daadwerkelijk actief in week X.
// Kan stijgen én dalen — dat is precies het punt, in tegenstelling tot een
// cumulatieve som die alleen maar kan groeien en niks zegt over het huidige
// moment.
// ══════════════════════════════════════════════════════════════════════════
function renderCamPerWeekChart() {
  const el = document.getElementById('camPerWeekChart');
  if (!el) return;
  const alle = [...camerasActief, ...camerasGeschiedenis];
  const alleStarts = alle.flatMap(c => (c.periodes || []).map(p => p.start)).filter(Boolean);
  if (!alleStarts.length) { el.innerHTML = '<div class="viz-empty">Geen camera-data</div>'; return; }

  const vroegste = alleStarts.sort()[0];
  const start = new Date(vroegste + 'T00:00:00');
  const vandaag = new Date();
  const weken = [];
  for (let d = new Date(start); d <= vandaag; d.setDate(d.getDate() + 7)) {
    weken.push(d.toISOString().slice(0, 10));
  }
  if (weken.length < 2) { el.innerHTML = '<div class="viz-empty">Onvoldoende data — minder dan 2 weken sinds eerste camera</div>'; return; }

  const data = weken.map(weekStart => {
    const aantalActief = alle.filter(c => (c.periodes || []).some(p => p.start <= weekStart && (p.eind || p.start) >= weekStart)).length;
    return { maand: weekStart, count: aantalActief };
  });
  tekenLijnGrafiek('camPerWeekChart', data, 'Camera\'s actief');
}
...
// Zelfde keyword-classificatie-stijl als voorheen bij de onderwerpenranglijst,
// toegepast op het bestaande excerpt-veld — geen scraper-wijziging nodig, data lag er al.
// ══════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════
// NIEUW — WONINGSLUITINGEN DIE NU NOG GESLOTEN ZIJN
// Spiegelbeeld van "camera's — nu actief": alleen sluitingen met een
// (bekende of geschatte) einddatum die nog niet gepasseerd is. Sluitingen
// zonder einddatum kunnen we niet met zekerheid als "nog gesloten" of
// "waarschijnlijk voorbij" classificeren — die tonen we apart, expliciet
// gelabeld als onbekend, in plaats van ze stilzwijgend mee of niet mee te
// tellen.
// ══════════════════════════════════════════════════════════════════════════
function renderWoningNuGeslotenLijst() {
  const el = document.getElementById('woningNuGeslotenLijst');
  if (!el) return;
  const vandaag = new Date().toISOString().slice(0, 10);

  const nogGesloten = woningsluitingen
    .filter(w => w.eind_datum && w.eind_datum >= vandaag)
    .sort((a, b) => a.eind_datum.localeCompare(b.eind_datum));
  const onbekendeStatus = woningsluitingen.filter(w => !w.eind_datum);

  const rijenHtml = nogGesloten.map(w => {
    const dagenResterend = dagenTussen(vandaag, w.eind_datum);
    const bijnaVerlopen = dagenResterend <= 14;
    return `<div class="bk-item">
      <div class="bk-top"><div>
        <div class="bk-title-link" style="cursor:default;">🏠 ${esc(w.adres || w.titel)}</div>
        <div class="bk-meta">
          <span class="bk-date">gesloten sinds ${fmtDate(w.datum, 'full')}</span>
          <span class="badge ${bijnaVerlopen ? 'badge-hold' : 'badge-go'}">${dagenResterend} dagen resterend</span>
          <span class="badge badge-teal">${w.eind_datum_type === 'geschat_uit_artikeltekst' ? 'geschatte einddatum' : 'officiële einddatum'}</span>
        </div>
      </div></div>
    </div>`;
  }).join('');

  const onbekendHtml = onbekendeStatus.length
    ? `<div style="padding:10px 20px;font-size:11px;color:var(--muted);">+ ${onbekendeStatus.length} woningsluiting${onbekendeStatus.length === 1 ? '' : 'en'} zonder bekende einddatum — status onbekend, niet in bovenstaande telling meegenomen.</div>`
    : '';

  el.innerHTML = (nogGesloten.length ? rijenHtml : '<div class="empty">Geen woningsluitingen op dit moment nog actief (met bekende einddatum).</div>') + onbekendHtml;
}

// ══════════════════════════════════════════════════════════════════════════
// DUUR PER WONINGSLUITING — TABEL
// Vervangt het eerdere histogram: elke rij is één specifieke woningsluiting,
// zodat direct duidelijk is óm welke sluiting het gaat, niet alleen hoeveel
// er in een duur-bucket vallen.
// ══════════════════════════════════════════════════════════════════════════
function renderWoningsluitingDuurChart() {
  const el = document.getElementById('woningsluitingDuurChart');
  if (!el) return;
  if (!woningsluitingen.length) { el.innerHTML = '<div class="empty">Geen woningsluitingen beschikbaar</div>'; return; }

  const rijen = [...woningsluitingen]
    .sort((a, b) => (b.datum || '').localeCompare(a.datum || ''))
    .map(w => {
      const dagen = w.eind_datum ? dagenTussen(w.datum, w.eind_datum) : null;
      const duurLabel = dagen != null
        ? `${dagen} dagen${w.eind_datum_type === 'geschat_uit_artikeltekst' ? ' (geschat)' : ' (officieel)'}`
        : 'onbekend';
      return `<tr>
        <td style="padding:6px 8px;border-bottom:1px solid var(--rule);">${esc(w.adres || w.titel)}</td>
        <td style="padding:6px 8px;border-bottom:1px solid var(--rule);font-family:'JetBrains Mono',monospace;font-size:11px;white-space:nowrap;">${fmtDate(w.datum, 'short')}</td>
        <td style="padding:6px 8px;border-bottom:1px solid var(--rule);white-space:nowrap;">${duurLabel}</td>
        <td style="padding:6px 8px;border-bottom:1px solid var(--rule);">${w.eind_datum ? fmtDate(w.eind_datum, 'short') : '—'}</td>
        <td style="padding:6px 8px;border-bottom:1px solid var(--rule);"><a href="${w.link || '#'}" target="_blank" style="color:var(--teal);font-size:11px;">artikel →</a></td>
      </tr>`;
    }).join('');

  el.innerHTML = `<div style="overflow-x:auto;padding:0 4px;">
    <table style="width:100%;border-collapse:collapse;font-size:12px;">
      <thead><tr>
        <th style="text-align:left;padding:6px 8px;">Adres</th>
        <th style="text-align:left;padding:6px 8px;">Startdatum</th>
        <th style="text-align:left;padding:6px 8px;">Duur</th>
        <th style="text-align:left;padding:6px 8px;">Einddatum</th>
...
// NIEUW: kleurklasse voor een kruischeck-tekst — zowel de niet-AI kruischeck
// (kruischeck_claim() in scrape_collegeberichten.py) als een eventuele
// toekomstige AI-kruischeck geven platte Nederlandse tekst terug, dus dit
// classificeert op de tekst zelf, niet op een apart veld.
function kruischeckKlasse(tekst) {
  if (!tekst) return '';
  const t = tekst.toLowerCase();
  if (t.startsWith('bevestigd')) return 'bevestigd';
  if (t.startsWith('afwijkend') || t.includes('tegenstrijdig') || t.includes('tegenspr')) return 'weersproken';
  return 'neutraal'; // bv. "Gerelateerd raadsbesluit/motie gevonden" — geen bevestiging of ontkrachting, wel relevant
}
            ...
            : claims.map(c => {
                ...
                return `
                  <div class="cb-claim-item">
                    ...
                    ${c.kruischeck ? `<div class="cb-claim-kruischeck ${kruischeckKlasse(c.kruischeck)}">${esc(c.kruischeck)}</div>` : ''}
                  </div>`;
              }).join('')
          }
        </div>
...
// ══════════════════════════════════════════════════════════════════════════
// NIEUW — "SINDS WANNEER"-LABEL PER GRAFIEK
// Zoekt zelf de vroegste datum in percentageHistorie waarop een gegeven veld
// voor het eerst aanwezig is (bv. 'per_lijn' pas vanaf 27 juli, 'totaal_per_lijn'
// pas vanaf 3 augustus). Geen hardcoded datums — het label groeit vanzelf mee
// als de scraper ooit verder terug data krijgt of méér gaat vastleggen.
// ══════════════════════════════════════════════════════════════════════════
function eersteDatumMetVeld(veldNaam) {
  const datums = Object.keys(percentageHistorie).filter(d => percentageHistorie[d][veldNaam] != null).sort();
  return datums[0] || null;
}
function labelSindsDatum(datumStr) {
  if (!datumStr) return '';
...
function actievePercentageHistorie() {
  // Een gekozen maand heeft voorrang boven de compleet/alles-knoppen: de
  // dropdown biedt toch alleen maanden aan die binnen "volledige dekking"
  // vallen, dus een extra ondergrens is dan niet nodig.
  if (_uvMaandFilter) {
    return Object.fromEntries(
      Object.entries(percentageHistorie).filter(([datum]) => datum.slice(0, 7) === _uvMaandFilter)
    );
  }
...
function renderUvDekkingsFilterButtons() {
  ...
  el.innerHTML = `
    <button type="button" class="filter-select"
      style="cursor:pointer;${!_uvMaandFilter && _uvDekkingsFilter === 'compleet' ? 'background:var(--teal);color:white;font-weight:700;' : ''}"
      onclick="setUvDekkingsFilter('compleet')">Vanaf volledige dekking (${grensLabel})</button>
    <button type="button" class="filter-select"
      style="cursor:pointer;${!_uvMaandFilter && _uvDekkingsFilter === 'alles' ? 'background:var(--teal);color:white;font-weight:700;' : ''}"
      onclick="setUvDekkingsFilter('alles')">Alles (incl. deelperiodes)</button>`;
  populateUvMaandFilter();
}
...
// NIEUW: dropdown om één specifieke maand te bekijken (alle 9 EBS-grafieken
// volgen deze keuze via actievePercentageHistorie()). Alleen maanden binnen
// "volledige dekking" worden aangeboden, want daarbuiten is de uitsplitsing
// per lijn/halte/oorzaak niet betrouwbaar (zie toelichting hierboven).
function populateUvMaandFilter() {
  const el = document.getElementById('uvMaandFilter');
  if (!el) return;
  const grens = volledigeDekkingVanaf();
  const maanden = new Set(
    Object.keys(percentageHistorie).filter(d => !grens || d >= grens).map(d => d.slice(0, 7))
  );
  const vandaagKey = Object.keys(totaalTeller)[0];
  if (vandaagKey && (!grens || vandaagKey >= grens)) maanden.add(vandaagKey.slice(0, 7));

  const lijst = [...maanden].sort().reverse();
  el.innerHTML = '<option value="">(hele periode)</option>' + lijst.map(m => {
    const [jaar, mnd] = m.split('-');
    const naam = MAAND_NAMEN_KORT_UV[parseInt(mnd, 10) - 1] || mnd;
    return `<option value="${m}">${naam} ${jaar}</option>`;
  }).join('');
  el.value = _uvMaandFilter;
}

function setUvMaandFilter(waarde) {
  _uvMaandFilter = waarde;
  if (waarde) _uvDekkingsFilter = 'compleet'; // een specifieke maand valt altijd binnen de volledige dekking
  renderUitval();
}
